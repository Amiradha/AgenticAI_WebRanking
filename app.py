...
def get_webreinvent_rank(keyword):
    driver = None
    try:
        options = Options()
        ...
        for index, result in enumerate(results, start=1):
            ...
            if link and 'webreinvent.com' in link['href']:
                return {
                    "rank": index,
...
def run_batch_search(keywords, task_id):
    try:
        ...
        for keyword in keywords:
            result = get_webreinvent_rank(keyword)
...
def export_data():
    try:
        ...
        export_path = os.path.join("ranking_data", "webreinvent_rankings_export.csv")
        df = pd.DataFrame(results)
        df.to_csv(export_path, index=False)
        
        return send_file(export_path, as_attachment=True, download_name='webreinvent_rankings.csv')
    except Exception as e:
